...
class GUI(threading.Thread):
    ...
    def register(self, register):
        self.client.notify_server(register, 'register')
...
class LoginWindow(Window):
    ...
    def build_window(self):
        ...
        self.label = tk.Label(self.root, text='Username', width=20, font=self.font)
        ...
        self.label = tk.Label(self.root, text='Password', width=20, font=self.font)
        self.label.pack(side=tk.LEFT, expand=tk.YES)

        self.entry2 = tk.Entry(self.root, width=20, font=self.font, show="*")
        self.entry2.focus_set()
        self.entry2.pack(side=tk.LEFT)
        self.entry2.bind('<Return>', self.get_login_event)
        ...
        self.button2 = tk.Button(self.root, text='Register', font=self.font)
        self.button2.pack(side=tk.LEFT)
        self.button2.bind('<Button-1>', self.get_register_event)
    ...
    def get_login_event(self, event):
        ...
        if len(self.user) == 0 or len(self.password) == 0:
            self.gui.display_alert("Wrong Username or Password")
        else:
            self.login = self.user + ";" + self.password
            self.gui.notify_server(self.login,"login")
            self.login = self.user
    ...
    def get_register_event(self, event):
        ...
        if len(self.username) == 0 or len(self.password) == 0:
            self.gui.display_alert("Empty boxes")
        else:
            self.login = self.username + ";" + self.password
            self.gui.notify_server(self.login,"register")
            self.login = self.username
            # self.root.quit()
